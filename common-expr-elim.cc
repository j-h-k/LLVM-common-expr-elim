...
private:
	...
	//std::unordered_map<std::string, std::pair<std::string, llvm::Value*>> available_expressions_detailed;
...
public:
	...
	// Replaces all apperances of repeated instruction with already in available_expressions.
	void replaceExpression(llvm::Instruction *&instruction, std::string &key) {
		...
		llvm::ReplaceInstWithValue(instruction->getParent()->getInstList(), 
						ii, 
						replacewiththis);
	}
			...
			++basic_block)
		{
				...
				/*Read below important notice.*/)
			{
				/*
 				 ...
 				 * a segfault would arise. This is because when replaceExpression is called
 				 * the llvm::ReplaceInstWithValue function erases the instruction from
...
// optimizer tool.
static llvm::RegisterPass<CommonExprElimPass> X("common-expr-elim",
	"My function pass",
	false,
	false);
